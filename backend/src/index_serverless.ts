...
export default async function createServer() {
  ...
  async function processImageGeneration(taskId: string, templateId: string, imageUrl: string) {
    try {
      ...
      await prisma.task.update({ where: { id: taskId }, data: { status: 'COMPLETED', outputUrl } });
    } catch (error: any) {
      await prisma.task.update({ where: { id: taskId }, data: { status: 'FAILED', errorMessage: error.message } });
    }
  }
  ...
  server.post('/api/auth', async (request, reply) => {
    ...
    if (!user) {
      ...
      user = await prisma.user.create({ data: { telegramId, username: tgUser.username, referredBy, tokens: 3 } });
    }
  ...
  server.post('/api/generate/create-task', { preHandler: [verifyTMAAuth] }, async (request, reply) => {
    ...
    const task = await prisma.task.create({ data: { userId, templateId, inputUrl: imageUrl, status: 'PENDING' } });
  ...
  server.post('/api/webhooks/telegram', async (request) => {
    ...
    if (body.message?.text === '/start') {
      ...
      await axios.post(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, {
        chat_id: chatId,
        text: 'Привет! Нажми кнопку ниже, чтобы запустить PicCreate.',
        reply_markup: {
          inline_keyboard: [[{ text: '🚀 Запустить PicCreate', web_app: { url: miniAppUrl } }]]
        }
      });
    }
    if (body.message?.successful_payment) {
      const payload = JSON.parse(body.message.successful_payment.invoice_payload);
      await prisma.user.update({ where: { telegramId: BigInt(body.message.from.id) }, data: { tokens: { increment: payload.tokens } } });
    }
    return { ok: true };
  });
